...
- **URL 必须从 web_search 结果原样复制**，粘贴搜索返回的真实链接。
- **禁止**：缩写 URL、简化路径、改写/猜测 article ID、去掉 query string、把长链接「美化」成短域名。
  - 反例：真实的 `article71342539.ece` 落盘写成 `article68501234.ece` → 404。
- **禁止编造 URL**：搜不到精确来源时，宁可标「来源待核」，也绝不写一个看起来合理但实际不存在的链接。
- **落盘后自检**：抽查 3-5 条链接，逐字符比对搜索返回原文；发现不一致立即更正。
- 域名级备用链接（如 `https://www.reuters.com/`）只作为补充，不能替代具体文章 URL。
